...
public abstract class McuManager {
	 ...
	 */
	public <T extends McuMgrResponse> void send(int op, int flags, int len, int groupId, int sequenceNum, int commandId,
					 Map<String, Object> payloadMap, Class<T> respType, McuMgrCallback<T> callback) {
		try {
			if (len == 0 && !getScheme().isCoap()) {
				len = CBOR.toBytes(payloadMap).length;
			}
			Log.d(TAG, String.format(Locale.getDefault(),
					"Building header with op: %d, flags: %d, len: %d, group: %d, sequence: %d, id: %d",
					op, flags, len, groupId, sequenceNum, commandId));
